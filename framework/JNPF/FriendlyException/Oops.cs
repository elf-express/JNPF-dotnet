...
public static class Oops
{
    ...
    public static AppFriendlyException Oh(object errorCode, params object[] args)
    {
        ...
        var friendlyException = new AppFriendlyException(Message, errorCode) { ErrorCode = ErrorCode, Args = args };
    ...
    private static string MontageErrorMessage(string errorMessage, string errorCode, bool? hideErrorCode, params object[] args)
    {
        ...
        if (App.HttpContext != null)
        {
            ...
            heraderDic.Add("Authorization", App.HttpContext.Request.Headers["Authorization"]);
            heraderDic.Add("Accept-Language", App.HttpContext.Request.Headers["Accept-Language"]);
            heraderDic.Add("JNPF_API", true);
            var result = string.Format("http://{0}/api/system/BaseLang/GetLanguageText/{1}", App.HttpContext.Request.Host, errorCode).SetHeaders(heraderDic).GetAsStringAsync().Result;
            if (!string.IsNullOrEmpty(result))
            {
                dynamic? clay = Clay.Parse(result);
                realErrorMessage = clay.data;
                if (string.IsNullOrEmpty(realErrorMessage))
                {
                    realErrorMessage = errorMessage.Render();
                }
            }
        }
