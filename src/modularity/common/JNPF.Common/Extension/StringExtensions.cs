...
public static class StringExtensions
{
    ...
    /// <param name="source"></param>
    /// <returns></returns>
    public static List<string> Substring4(this string source)
    {
        MatchCollection mc = Regex.Matches(source, @"@\{(.*?)\}");
        return mc.Cast<Match>().Select(m => m.Value.TrimStart('@').TrimStart('{').TrimEnd('}')).ToList();
    }
    ...
    /// 是否是金额.
    /// </summary>
    public static bool IsAmount(this string value)
    {
    ...
    /// </summary>
    /// <param name="value"></param>
    public static bool IsCarNumber(this string value)
    {
        ...
        return Regex.IsMatch(value, carnumRegex);
    }
